...
def replace_inputs(line_arr, input_dict):
    replace_line_arr = []
    for i,line in enumerate(line_arr):
...
def add_gates(replace_lin_arr, input_dict, insert_id):
    #Add inputs first
...
def replace_vdd(line_arr, input_dict, output_dict):
    ...
    output_list = []
    #First for loop to search for vdd
    for i, line in enumerate(line_arr):
...
final_lin_arr, insert_id = replace_inputs(replace_lin_arr, input_dict)

#* STEP 3: Add two gates
add_gates(final_lin_arr, input_dict, insert_id)

#* STEP 4: Final Write
file_writer(os.path.join(cwd, sys.argv[1][:-6] + "_enc.bench"), final_lin_arr)